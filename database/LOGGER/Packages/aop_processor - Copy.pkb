...
BEGIN
  check_timeout;
  RETURN REGEXP_REPLACE(REGEXP_SUBSTR(g_code,i_search,g_current_pos,1,i_modifier),'^\s|$\s','');
END;
...
BEGIN
  check_timeout;
  g_current_pos := REGEXP_INSTR(g_code,i_search,g_current_pos,1,1,'i');
  IF g_current_pos = 0 then
    raise x_string_not_found;
  end if;
END;
...
BEGIN
  check_timeout;
  g_current_pos := REGEXP_INSTR(g_code,i_search,g_current_pos,1,0,'i');
  IF g_current_pos = 0 then
    raise x_string_not_found;
  end if;
END;
...
BEGIN
  ...
  loop
    l_keyword := UPPER(TRIM(REGEXP_SUBSTR(g_code,'\sDECLARE\s|\sBEGIN\s|\sEND;\s',g_current_pos,1,'i')));
	--EXIT WHEN l_keyword = 'END;';
...
BEGIN
  ...
  --Find node type
  l_keyword := UPPER(TRIM(REGEXP_SUBSTR(g_code,'\sPROCEDURE\s|\sFUNCTION\s',g_current_pos,1,'i')));
  ms_logger.note(l_node, 'l_keyword' ,l_keyword);   
  CASE 
...
---------------------------------------------------------------------------------
  ...
  begin
	...
	l_end_pos     := 1;
	LOOP
	  l_end_pos := REGEXP_INSTR(g_code,L_END_MASK,l_current_pos,1,0,'i');
	...
	begin
	  g_current_pos := 1;
	  l_keyword := UPPER(TRIM(REGEXP_SUBSTR(g_code,'\sDECLARE\s|\sBEGIN\s|\sPROCEDURE\s|\sFUNCTION\s|\sPACKAGE BODY\s',g_current_pos,1,'i')));
	  ms_logger.note(l_node, 'l_keyword' ,l_keyword);
	  ...
	  CASE 
		...
		WHEN l_keyword = 'PACKAGE BODY' THEN
		  ...
		  l_package_name := get_next_lower(g_word_search);
          --declaration of package body
          AOP_declare(i_level => 1);	
		  --initialisation of package body (optional)
          l_keyword := UPPER(TRIM(REGEXP_SUBSTR(g_code,'\sBEGIN\s|\END;\s',g_current_pos,1,'i')));
